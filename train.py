...
def run_experiment2(opt_name="SGD", num_epochs=10, params_list=None):
    ...
    for i, params in enumerate(params_list):
        ...
        if opt_name == "SGD":
            optimizer = optim.SGD(model.parameters(), lr=params['lr'], momentum=params.get('momentum', 0.9), weight_decay=5e-4)
            scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1)  
        elif opt_name == "Adam":
            optimizer = optim.Adam(model.parameters(), lr=params['lr'], weight_decay=5e-4, amsgrad=True)
            scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.5)
        elif opt_name == "SAM":
            base_optimizer = optim.SGD
        ...
        for epoch in range(num_epochs):
            ...
            print(f"[{opt_name} Config {i+1}] Epoch {epoch+1:2d}, "
                  f"Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.2f}%, "
                  f"Test Loss: {test_loss:.4f}, Test Acc: {test_acc:.2f}%")
        ...
        np.savetxt(f'{config_id}_metrics.txt', data, delimiter=',', 
                   header='train_loss,train_acc,test_loss,test_acc', comments='')
